...
class FcmService {
  ...
  /// Clears the FCM token locally and detaches it from the user's UUID in the backend
  /// to prevent push notification bleed across different sessions/logouts.
  Future<void> clearDeviceToken() async {
    try {
      final userUuid = UserData.instance.uuid;
      if (userUuid.isNotEmpty) {
        log('Detaching FCM Token from backend user UUID: $userUuid');
        await _repository.updateFcmToken(userUuid, '');
      }
      await FirebaseMessaging.instance.deleteToken();
      log('Deleted local FCM device token.');
    } catch (e) {
      log('FCM Token Clear error: $e');
    }
  }
}
