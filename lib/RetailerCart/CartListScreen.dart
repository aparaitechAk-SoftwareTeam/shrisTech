...
class _CartListScreenState extends State<CartListScreen>
  ...
  Future<void> _order(CartItemModel item) async {
    ...
    try {
      ...
      // FIX #5: Sync local cart state after checkout transition
      setState(() {
        _cart = _cartWithoutItem(item.cartItemId);
      });
      // Invalidate global cart cache
      RetailerCartService.instance.clear();
      
      _snack('Success! ${item.product.productName} ordered.');
    } catch (error) {
      if (!mounted) return;
      _snack(_errorText(error), success: false);
    } finally {
      if (mounted) {
        setState(() => _busyIds.remove(item.cartItemId));
        // Verify state sync with backend silently
