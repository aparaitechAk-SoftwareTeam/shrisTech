      ...
      builder: (context, constraints) {
        ...
        if (isLoading) {
          return Row(
            children: [
              Expanded(
                child: ShimmerCard(height: isCompact ? 72 : 82),
              ),
...
class _BannerCarouselState extends State<BannerCarousel>
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.banners.isEmpty) {
      return const EmptyWidget(message: RetailerHomeText.noData);
    }
  ...
  Widget build(BuildContext context) {
    ...
    switch (value.status) {
      ...
      case RetailerSectionStatus.empty:
        return EmptyWidget(message: value.message ?? RetailerHomeText.noData);
